...
//week4 test
int pmdEDUMgr::startEDU ( EDU_TYPES type, void* arg, EDUID *eduid ,int flag )
{
   int     rc = EDB_OK ;
   EDUID     eduID = 0 ;
   pmdEDUCB* eduCB = NULL ;
   ...
   if ( true == _idleQueue.empty () || !isPoolable ( type ) )
   {
      ...
      //week4 test
      setFlag(flag);
      rc = _createNewEDU ( type, arg, eduid,flag ) ;
      if ( EDB_OK == rc )
         goto done ;
      goto error ;
   }
...
int pmdEDUMgr::_createNewEDU ( EDU_TYPES type, void* arg, EDUID *eduid,int flag )
{
   int rc               = EDB_OK ;
